...
function Search({ query, setQuery, result, setResult }) {
  ...
  const URL =
    "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyDQoi_C7sxb1WQox8JSERaEUr1L3oHG0b0";
  ...
  async function callApi() {
    ...
    setResult((prev) => [
      ...prev,
      { question: query, answer: data.candidates[0].content.parts[0].text },
    ]);
  }
  ...
  return (
    <div className="flex items-center justify-between  sm:w-7/12   h-10 rounded-2xl  border-2 border-pink-500">
      ...
      <div className="mr-5 text-purple-700">
        <button>
          <IoMdSend size={25} onClick={callApi} />
        </button>
      </div>
    </div>
  );
